...
```{r vfs, echo=TRUE, fig.show="hold"}
	...
	vfs.CL <- VFSsim(nyears = 5, thissoil = subset(soildat, Soil == "CL"), rain=rain.compare, temperature=temp.compare, thisbuffer = subset(bufferdat, Species == "bluegrass"), Duration = 2, FieldArea = 4000, VFSwidth = 10.7, VFSslope = 0.02, z = 1000, b = 1.5)
...
```{r erosion, echo=TRUE, fig.show="hold"}
	...
	erosion.CL <- VFSsim(nyears = 5, thissoil = subset(soildat, Soil == "CL"), rain=rain.compare, temperature=temp.compare, thisbuffer = NA, Duration = 2, FieldArea = 4000, VFSwidth = 10.7, VFSslope = 0.02, z = 1000, b = 1.5)
...
# Run the model for all twelve soil texture classes with default arguments
	vfs.AllSoil <- lapply(seq_len(nrow(soildat)), function(i)VFSsim(nyears = 5, thissoil = soildat[i,], rain=rain.compare, temperature=temp.compare, thisbuffer = subset(bufferdat, Species == "bluegrass")))
